[Skip to main content](https://actualbudget.org/docs/api/#__docusaurus_skipToContent_fallback)
...
## Getting Started [​](https://actualbudget.org/docs/api/\#getting-started "Direct link to Getting Started")
...
### Connecting to a Remote Server [​](https://actualbudget.org/docs/api/\#connecting-to-a-remote-server "Direct link to Connecting to a Remote Server")
...
### Self-Signed Https Certificates [​](https://actualbudget.org/docs/api/\#self-signed-https-certificates "Direct link to Self-Signed Https Certificates")

If the serverURL is using [self-signed or custom CA certificates](https://actualbudget.org/docs/config/https), additional Node.js configuration will be needed for the connections to succeed.
...
- Option 1: Point environment variable [NODE\_EXTRA\_CA\_CERTS](https://nodejs.org/api/cli.html#node_extra_ca_certsfile) to the path of a file containing the public certificate.
- Option 2: Set environment variable [NODE\_TLS\_REJECT\_UNAUTHORIZED](https://nodejs.org/api/cli.html#node_tls_reject_unauthorizedvalue) to `0`. Not recommended if your program reaches out to any other endpoints other than the Actual server.
- Options 3: Use OpenSSL CA certificates configuration for Node and add your certificate to the OpenSSL SSL\_CERT\_DIR. What this requires depends on your build of Node.js, and the configuration details are beyond the scope of this documentation. See the [Node.js OpenSSL Strategy](https://github.com/nodejs/TSC/blob/main/OpenSSL-Strategy.md) page for a starting point.

## Writing Data Importers [​](https://actualbudget.org/docs/api/\#writing-data-importers "Direct link to Writing Data Importers")

If you are using another app, like YNAB or Mint, you might want to migrate your data into Actual. Right now, Actual officially supports [importing YNAB4 data](https://actualbudget.org/docs/migration/ynab4) and [importing nYNAB data](https://actualbudget.org/docs/migration/nynab) (and it works very well). But if you want to import all of your data into Actual, you can write a custom importer.

Note that this is not about importing transactions. If all you want to do is add transactions from a custom source (like your bank's API), use [`importTransactions`](https://actualbudget.org/docs/api/reference#importtransactions). In this context, a custom importer is something that takes _all_ of your data (budgets, transactions, payees, etc) and dumps them all into a new file in Actual.
...
This is very simple, but it takes some data in `my-data.json` and creates all the accounts and transactions from it. Functions used to convert items (like `convertAccount`) are not included here. Use the [reference docs](https://actualbudget.org/docs/api/reference) to learn the shape of objects that Actual expects.

**Note:** it's important that [`addTransactions`](https://actualbudget.org/docs/api/reference#addtransactions) is used here. You want to use it instead of [`importTransactions`](https://actualbudget.org/docs/api/reference#importtransactions) when dumping raw data into Actual. The former will not run the reconciliation process (which deduplicates transactions), and won't create the other side of transfer transactions, and more. If you use `importTransactions` it may adjust your data in ways that don't match the data you're importing.
...
## Methods [​](https://actualbudget.org/docs/api/\#methods "Direct link to Methods")
...
#### `init` [​](https://actualbudget.org/docs/api/\#init "Direct link to init")

`init({ stringdataDir, stringserverURL, stringpassword, booleanverbose }) → Promise<void>`
...
#### `shutdown` [​](https://actualbudget.org/docs/api/\#shutdown "Direct link to shutdown")
...
#### `utils.amountToInteger` [​](https://actualbudget.org/docs/api/\#utilsamounttointeger "Direct link to utilsamounttointeger")

`utils.amountToInteger(numberamount) → number`
...
#### `utils.integerToAmount` [​](https://actualbudget.org/docs/api/\#utilsintegertoamount "Direct link to utilsintegertoamount")

`utils.integerToAmount(numberamount) → number`

Convert an integer amount as used internally by Actual (such as `12345`) to the traditional floating point (i.e. `123.45`).

- [Getting Started](https://actualbudget.org/docs/api/#getting-started)
  - [Connecting to a Remote Server](https://actualbudget.org/docs/api/#connecting-to-a-remote-server)
  - [Self-Signed Https Certificates](https://actualbudget.org/docs/api/#self-signed-https-certificates)
- [Writing Data Importers](https://actualbudget.org/docs/api/#writing-data-importers)
- [Methods](https://actualbudget.org/docs/api/#methods)