...
function buildSystemPrompt(context: BudgetContext): string {
  ...
  parts.push(
    'You are a helpful personal finance assistant for the Actual Budget app. ' +
      ...
      '- "create-schedules-batch": params: {schedules: [{name, payee_name, accountId?, amount, amountOp?, date, frequency, interval?, posts_transaction?}]} — Create multiple schedules at once. accountId is optional (defaults to first open account). name is REQUIRED for each entry — use the payee name. date is REQUIRED — calculate the next expected occurrence from the last charge date + frequency. Do NOT use today\'s date unless it truly is the next occurrence.\n' +
      ...
      '- If the user agrees, IMMEDIATELY emit a "create-schedules-batch" action block with the detected data. For each entry, ALWAYS include: name (use payee name), payee_name, amount (in cents, negative for expenses), frequency, and date (calculate next occurrence from lastDate + frequency — do NOT use today\'s date). Map frequencies: monthly→frequency:"monthly", weekly→frequency:"weekly", biweekly→frequency:"weekly" interval:2, quarterly→frequency:"monthly" interval:3, yearly→frequency:"yearly".\n' +
